...
| 4 | Firebase 项目（dev） | **运维底座**：远程配置 / 缺陷定位（及无友盟时的埋点） | ☐ | | | 可暂不申请；有友盟 key 时埋点优先友盟 |
| 5 | Firebase 项目（prod） | 生产环境运维底座 | ☐ | | | 可与 dev 同项目不同 App，或分项目 |
| 5b | 友盟 U-App（iOS/Android AppKey） | **友盟统计**（行为埋点主适配） | ☐ | | | 写入 `umeng.*`；勿提交 |
| 5c | App Store / 应用市场链接 | **应用内评分提示**好评跳转 | ☐ | | | 写入 `store.*` |
...
| `dart_defines.prod.json` | 本地 / CI prod；Fastlane 发布 |

### 4.2 全部 dart-define 键
...
| `UMENG_ANDROID_APP_KEY` | 空 | 接友盟时 ✅ | 来自 `umeng.android_app_key` |
| `UMENG_IOS_APP_KEY` | 空 | 接友盟时 ✅ | 来自 `umeng.ios_app_key` |
| `UMENG_CHANNEL` | `Flutter` | | 来自 `umeng.channel` |
| `IOS_STORE_URL` | 空 | 评分好评跳转 | 来自 `store.ios_url` |
| `ANDROID_STORE_URL` | 空 | 评分好评跳转 | 来自 `store.android_url` |
