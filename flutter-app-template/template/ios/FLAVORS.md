...
`Debug-dev` / `Release-prod` / … configurations. CocoaPods **must** list those
same names in `ios/Podfile` (`project 'Runner', { ... }`), otherwise Xcode looks
for `Pods-Runner-frameworks-Debug-dev-*.xcfilelist` with an empty `PODS_ROOT`.
...
cd /path/to/instance/ios && pod install
```
